...
Vector AimHelpers::calculateRelativeAngle(const Vector& source, const Vector& destination, const Vector& viewAngles) noexcept
{
    return ((destination - source).toAngle() - viewAngles).normalize();
}

bool AimHelpers::handleTaserPenetration(UserCmd* cmd, Vector& angle, Vector& target) noexcept
{
    Vector end;
    Trace enterTrace;
    __asm {
    ...
    if (sqrt(sqrt(enterTrace.startpos.x * enterTrace.startpos.y * enterTrace.startpos.z)) - sqrt(sqrt(target.x * target.y * target.z)) <= config->zeusbot.autoZeusMaxPenDist)
        return true;
    else
        return false;
}
...
bool AimHelpers::hitChance(Entity* localPlayer, Entity* entity, StudioHitboxSet* set, const matrix3x4 matrix[MAXSTUDIOBONES], Entity* activeWeapon, const Vector& destination, const UserCmd* cmd, const int hitChance) noexcept
{
...
bool AimHelpers::hitChanceWeapon(Entity* localPlayer, Entity* entity, Entity* weaponData, const Vector& destination, const UserCmd* cmd, const int hitChance) noexcept
{
    static auto isSpreadEnabled = interfaces->cvar->findVar("weapon_accuracy_nospread");
    if (!hitChance || isSpreadEnabled->getInt() >= 1)
        return true;
    ...
    for (int i = 0; i < maxSeed; ++i)
    {
        ...
        const auto weaponIndex = weaponData->itemDefinitionIndex();
...
void AimHelpers::calculateArmorDamage(float armorRatio, int armorValue, bool hasHeavyArmor, float& damage) noexcept
{
...
bool AimHelpers::canScan(Entity* entity, const Vector& destination, const WeaponInfo* weaponData, int minDamage, bool allowFriendlyFire) noexcept
{
    if (!localPlayer)
        return false;
...
float AimHelpers::getScanDamage(Entity* entity, const Vector& destination, const WeaponInfo* weaponData, int minDamage, bool allowFriendlyFire) noexcept
{
    if (!localPlayer)
        return 0.f;
...
bool AimHelpers::hitboxIntersection(const matrix3x4 matrix[MAXSTUDIOBONES], int iHitbox, StudioHitboxSet* set, const Vector& start, const Vector& end) noexcept
{
...
std::vector<Vector> AimHelpers::multiPoint(Entity* entity, const matrix3x4 matrix[MAXSTUDIOBONES], StudioBbox* hitbox, Vector localEyePos, int _hitbox, int _multiPoint)
{
    ...
    Vector currentAngles = AimHelpers::calculateRelativeAngle(center, localEyePos, Vector{});
    ...
    float multiPoint = (minM(_multiPoint, 95)) * 0.01f;
