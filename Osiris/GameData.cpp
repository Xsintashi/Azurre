...
#include "Resources/dance.h"
#include "Resources/logo.h"
#include "Resources/anime.h"
#include "Resources/banner.h"
#include "Resources/grenades.h"
#include "Resources/dbg.h"
...
void GameData::createNeededThirdPartyFiles() {
    if (!interfaces->engine->getGameDirectory())
        return;

    const char* getGameDirectory = interfaces->engine->getGameDirectory();
    std::string azurreGameDir = std::string(getGameDirectory).append("\\azurre\\menuPanorama");

    std::filesystem::create_directories(azurreGameDir);

    std::ofstream default_res(azurreGameDir + "\\default.res", std::ios::binary);
    if (default_res.is_open())
    {
        default_res.write((const char*)Resource::default_res, sizeof(Resource::default_res));
        default_res.close();
    }

    std::ofstream forniteDances_res(azurreGameDir + "\\forniteDances.res", std::ios::binary);
    if (forniteDances_res.is_open())
    {
        forniteDances_res.write((const char*)Resource::forniteDances_res, sizeof(Resource::forniteDances_res));
        forniteDances_res.close();
    }

    std::ofstream fortnite_emotes_v2_dx90_vtx(azurreGameDir + "\\fortnite_emotes_v2.dx90.vtx", std::ios::binary);
    if (fortnite_emotes_v2_dx90_vtx.is_open())
    {
        fortnite_emotes_v2_dx90_vtx.write((const char*)Resource::fortnite_emotes_v2_dx90_vtx, sizeof(Resource::fortnite_emotes_v2_dx90_vtx));
        fortnite_emotes_v2_dx90_vtx.close();
    }

    std::ofstream fortnite_emotes_v2_mdl(azurreGameDir + "\\fortnite_emotes_v2.mdl", std::ios::binary);
    if (fortnite_emotes_v2_mdl.is_open())
    {
        fortnite_emotes_v2_mdl.write((const char*)Resource::fortnite_emotes_v2_mdl, sizeof(Resource::fortnite_emotes_v2_mdl));
        fortnite_emotes_v2_mdl.close();
    }

    std::ofstream fortnite_emotes_v2_vvd(azurreGameDir + "\\fortnite_emotes_v2.vvd", std::ios::binary);
    if (fortnite_emotes_v2_vvd.is_open())
    {
        fortnite_emotes_v2_vvd.write((const char*)Resource::fortnite_emotes_v2_vvd, sizeof(Resource::fortnite_emotes_v2_vvd));
        fortnite_emotes_v2_vvd.close();
    }

    std::ofstream info_txt(azurreGameDir + "\\into.txt", std::ios::binary);
    info_txt << "Azurre\n" << "This folder contains files needed to work properly.";
    info_txt.close();
}

constexpr float playerVisibilityUpdateDelay = 0.1f;
    ...
    for (int i = 1; i <= highestEntityIndex; ++i) {
        ...
        if (entity->isPlayer()) {
            ...
            if (entity->isWeapon()) {
                ...
                if (classIdInt == dynamicClassId->fogController && !Visuals::shouldRemoveFog())
                {
                    const auto fog = reinterpret_cast<FogController*>(entity);
                    unsigned char _color[3];

                    if (Visuals::FogColor().asColor3().rainbow)
                    {
                        const auto [colorR, colorG, colorB] { rainbowColor(Visuals::FogColor().asColor3().rainbowSpeed) };
                    ...
                    {
                        _color[0] = std::clamp(static_cast<int>(Visuals::FogColor().asColor3().color[0] * 255.0f), 0, 255);
                        _color[1] = std::clamp(static_cast<int>(Visuals::FogColor().asColor3().color[1] * 255.0f), 0, 255);
                        _color[2] = std::clamp(static_cast<int>(Visuals::FogColor().asColor3().color[2] * 255.0f), 0, 255);
                    }

                    const unsigned long color = *(unsigned long*)_color;

                    fog->enable() = Visuals::FogEnabled() ? 1 : 0;
                    fog->start() = Visuals::FogStart();
                    fog->end() = Visuals::FogEnd();
                    fog->density() = Visuals::FogDensity();
                    fog->color() = color;
                }
            }
        }
...
void LocalPlayerData::update() noexcept
{
    ...
    const Vector end = start + Vector::fromAngle(localPlayer->eyeAngles()) * 1000.0f;
    Trace trace;
    interfaces->engineTrace->traceRay({ start, end }, 0x80040FF, localPlayer.get(), trace);
...
void PlayerData::update(Entity* entity) noexcept
{
    ...
    velocity = entity->velocity().length2D();
    maxDesyncAngle = entity->getMaxDesyncAngle();
    angle = entity->eyeAngles();
    isScoping = entity->isScoped();
    hasHelmet = entity->hasHelmet();
    hasDefuse = entity->hasDefuser();
    money = entity->money();
    index = entity->index();
    duckAmount = entity->duckAmount();
    duckSpeed = entity->duckSpeed();
    const auto idx = entity->index();
    if (const auto pr = *memory->playerResource) {
        armor = pr->armor()[idx];
        competitiveWins = pr->competitiveWins()[idx];
        competitiveRank = pr->competitiveRanking()[idx];
        hasBomb = (idx == pr->BombCarrier());

        if (const auto clantag = pr->getClan(idx); clantag && clantag[0] != '\0')
    ...
    isFakeDucking = entity->duckSpeed() = 8.f && (entity->duckAmount() > 0.01f && entity->duckAmount() <= 0.9f) && entity->isOnGround();
...
static const PNGTexture logo{ Resource::logo };
static const PNGTexture anime{ Resource::anime };
static const PNGTexture banner{ Resource::banner };
static const PNGTexture debugPic{ Resource::dbg };

static const PNGTexture smoke{ Resource::smoke };
static const PNGTexture molotov{ Resource::molotov };
static const PNGTexture highExplosive{ Resource::he };
...
ImTextureID GameData::getLogo() {
    return logo.getTexture();
}

ImTextureID GameData::getNade(int type) {
    /*Types:
        0 - Grenade
        1 - Flash
        2 - Smoke
    ...
    */
    ...
    default:
    case 0:
        return highExplosive.getTexture();
    case 1:
        return flash.getTexture();
    case 2:
        return smoke.getTexture();
    case 3:
        return molotov.getTexture();
    case 4:
        return decoy.getTexture();
    }
}

ImTextureID GameData::getAnime() {
    return anime.getTexture();
}

ImTextureID GameData::getBaner() {
    return banner.getTexture();
}

ImTextureID GameData::getDebug() {
    return debugPic.getTexture();
}
