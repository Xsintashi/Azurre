...
void Tickbase::start(UserCmd* cmd) noexcept
{
    ...
    if (!Extra::isDTEnabled() && !Extra::isHideShotsEnabled())
    {
        if (hasHadTickbaseActive)
            shift(cmd, ticksAllowedForProcessing, true);
        hasHadTickbaseActive = false;
    ...
    if (Extra::isDTEnabled())
        targetTickShift = 13;
    else if (Extra::isHideShotsEnabled())
        targetTickShift = 9;
...
bool Tickbase::canRun() noexcept
{
    ...
    {
        ticksAllowedForProcessing = minD(ticksAllowedForProcessing++, maxUserCmdProcessTicks);
        chokedPackets = maxD(chokedPackets--, 0);
        pauseTicks++;
        return false;
    }
    return true;
...
bool Tickbase::canShift(int shiftAmount, bool forceShift) noexcept
{
    ...
    if (activeWeapon->isKnife() || activeWeapon->isGrenade() || activeWeapon->isBomb()
        || activeWeapon->itemDefinitionIndex() == WeaponId::Revolver
        || activeWeapon->itemDefinitionIndex() == WeaponId::Taser
        || activeWeapon->itemDefinitionIndex() == WeaponId::Healthshot)
        return false;
...
void Tickbase::resetTickshift() noexcept
{
    ...
    if (Extra::isTeleportEnabled())
        ticksAllowedForProcessing = maxD(ticksAllowedForProcessing - tickShift, 0);
    tickShift = 0;
}
