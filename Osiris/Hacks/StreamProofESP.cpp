...
static void drawArmorBar(const HealthBar& config, const ImVec2& pos, float height, int armor) noexcept
{
    if (!config.enabled)
        return;
    ...
    drawList->PushClipRect(pos + ImVec2{ 0.0f, (100 - armor) / 100.0f * height }, pos + ImVec2{ width + 1.0f, height + 1.0f });

    if (config.type == HealthBar::Gradient) {
        const auto tier3 = Helpers::calculateColor(205, 229, 255, 255);
        const auto tier2 = Helpers::calculateColor(170, 229, 255, 255);
        const auto tier1 = Helpers::calculateColor(128, 229, 255, 255);
        ...
        drawList->AddRectFilledMultiColor(ImFloor(min), ImFloor(max), tier3, tier3, tier2, tier2);
        min.y += height / 2.0f;
        max.y += height / 2.0f;
        drawList->AddRectFilledMultiColor(ImFloor(min), ImFloor(max), tier2, tier2, tier1, tier1);
    }
    else {
        const auto color = config.type == HealthBar::HealthBased ? Helpers::healthColor(std::clamp(armor / 100.0f, 0.0f, 1.0f)) : Helpers::calculateColor(config.asColor4());
        drawList->AddRectFilled(pos + ImVec2{ 1.0f, 1.0f }, pos + ImVec2{ width + 1.0f, height + 1.0f }, color & IM_COL32_A_MASK);
        drawList->AddRectFilled(pos, pos + ImVec2{ width, height }, color);
    }
...
static void renderPlayerBox(const PlayerData& playerData, const Player& config) noexcept
{
    ...
    drawArmorBar(config.armorBar, bbox.min - ImVec2{ 10.0f, 0.0f }, (bbox.max.y - bbox.min.y), playerData.armor);
    if (config.armorBar.enabled && playerData.armor < 100)
    {
        const auto position = bbox.min - ImVec2{ 5.0f, 0.0f } + ImVec2{ 0.0f, (100 - playerData.armor) / 100.0f * height };

        if (playerData.armor)
            renderText(playerData.distanceToLocal, config.textCullDistance, Color4(), std::to_string(playerData.armor).append(playerData.hasHelmet ? "+H" : "").c_str(), { position.x , position.y });
    }
    ...
    if (config.flags != 0) { // undefined config.flags value
        
        const int money = playerData.money;
        const bool fakeDuck = playerData.isFakeDucking;
        
        std::string flags;

        if(Helpers::getByteFromBytewise(config.flags, 0))
        flags += playerData.hasBomb ? "Has Bomb\n" : "";

        if(Helpers::getByteFromBytewise(config.flags, 1))
        flags += playerData.hasDefuse ? "Has Defuser\n" : "";

        if(Helpers::getByteFromBytewise(config.flags, 2))
        flags += playerData.isScoping ? "is Scoping\n" : "";

        flags += Helpers::getByteFromBytewise(config.flags, 3) ? std::string("$").append(std::to_string(money)).append("\n") : "";

        flags += Helpers::getByteFromBytewise(config.flags, 4) && fakeDuck ? std::string("FakeDuck\n") : "";

        const auto nameSize = renderText(playerData.distanceToLocal, config.textCullDistance, Color4(), flags.c_str(), { bbox.max.x + 16, bbox.min.y + 24} );
    }

    if (config.rank)
    {
        renderText(playerData.distanceToLocal, config.textCullDistance, Color4(), Helpers::ConvertRankFromNumber(true, playerData.competitiveRank), { (bbox.min.x + bbox.max.x) / 2, bbox.max.y - 2 });
    }
...
static void drawLineOfSight(const ColorToggleThickness& config, const PlayerData& playerData)
{
    ...
    const auto color = Helpers::calculateColor(config.asColorToggle());
...
void StreamProofESP::drawGUI(bool contentOnly) noexcept
{
    ...
    if (ImGui::BeginChild("##child", { 400.0f, 0.0f })) {
        ...
        if (currentCategory < 2) {
            ...
            ImGui::PopID();
            ImGui::SameLine(spacing);
            ImGui::Checkbox("Armor Bar", &playerConfig.armorBar.enabled);
            ImGui::SameLine();

            ImGui::PushID("Armor Bar");
            ...
            if (ImGui::BeginPopup("")) {
                ImGui::SetNextItemWidth(95.0f);
                ImGui::Combo("Type", &playerConfig.armorBar.type, "Gradient\0Solid\0");
                if (playerConfig.armorBar.type == HealthBar::Solid) {
                    ImGui::SameLine();
                    ImGuiCustom::colorPicker("", playerConfig.armorBar.asColor4());
                }
                ImGui::EndPopup();
            }
            ImGui::PopID();
            ...
            ImGui::PopID();
            ImGui::Checkbox("Rank", &playerConfig.rank);
            ImGui::SetNextItemWidth(100.f);
            ImGuiCustom::multiCombo("Flags", playerConfig.flags, "Bomb Carrier\0Defuse Kit Carrier\0Scoping\0Money\0FakeDuck\0");
            ImGui::Text("Int: %i", playerConfig.flags);
        }
