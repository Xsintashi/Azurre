...
void Glow::changeGlowThickness() noexcept
{
    const auto glowWidth = interfaces->cvar->findVar("glow_outline_width");
    glowWidth->setValue(glowOutlineWidth);
}
...
void Glow::render() noexcept
{
    ...
    for (int i = 0; i < memory->glowObjectManager->glowObjectDefinitions.size; i++) {
        ...
        auto applyPlayerGlow = [applyGlow](const std::string& name, Entity* entity) noexcept {
            const auto& gCfg = playerGlowConfig[name];
            if (gCfg.all.enabled)
                applyGlow(gCfg.all, entity->health());
            else if (gCfg.visible.enabled && entity->visibleTo(localPlayer.get()))
                applyGlow(gCfg.visible, entity->health());
            else if (gCfg.occluded.enabled && !entity->visibleTo(localPlayer.get()))
                applyGlow(gCfg.occluded, entity->health());
        };
...
void Glow::drawGUI(bool contentOnly) noexcept
{
    ...
    if (currentCategory <= 3) {
        ...
        ImGui::PopID();
        auto& gCfg = playerGlowConfig[categories[currentCategory]];
        switch (currentType) {
        case 0: currentItem = &gCfg.all; break;
        case 1: currentItem = &gCfg.visible; break;
        case 2: currentItem = &gCfg.occluded; break;
        }
    } else {
    ...
    ImGui::SameLine();
    ImGui::Toggle("Enabled", &currentItem->enabled);
    ImGui::Separator();
    ImGui::Columns(2, nullptr, false);
    ImGui::SetColumnOffset(1, 150.0f);
    ImGui::Toggle("Health based", &currentItem->healthBased);
    ...
    ImGui::SliderFloat("", &glowOutlineWidth, 0.0f, 100.0f, "Thickness: %.2f");
...
json Glow::toJson() noexcept
{
    ...
    to_json(j["Glow thickness"], glowOutlineWidth, {});
    return j;
}
...
void Glow::fromJson(const json& j) noexcept
{
    ...
    read(j, "Glow thickness", glowOutlineWidth);
}
