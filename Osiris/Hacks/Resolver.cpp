...
void Resolver::processMissedShots() noexcept
{
	...
	const auto angle = AimHelpers::calculateRelativeAngle(snapshot.eyePosition, snapshot.bulletImpact, Vector{ });
	...
	{
		if (AimHelpers::hitboxIntersection(matrix, hitbox, set, snapshot.eyePosition, end))
		{
			resolverMissed = true;
			...
			Notifications::addNotification(Log::LogColor(), missed.c_str());
			Animations::setPlayer(snapshot.playerIndex)->misses++;
			break;
		}
	}
	if (!resolverMissed)
		Notifications::addNotification(Log::LogColor(), "Missed due to spread");
}
