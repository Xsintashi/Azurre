...
void Animations::renderStart(csgo::FrameStage stage) noexcept
{
...
{
    if (Backtrack::fakeLatencyAmountRecive() <= 0)
        return 0.f;
    return static_cast<float>(Backtrack::fakeLatencyAmountRecive()) / 1000.f;
}

void Animations::handlePlayers(csgo::FrameStage stage) noexcept
{
    static auto gravity = interfaces->cvar->findVar("sv_gravity");
    const float timeLimit = static_cast<float>(Backtrack::TimeLimitRecive()) / 1000.f + getExtraTicks();
    ...
    for (int i = 1; i <= interfaces->engine->getMaxClients(); i++)
    {
        ...
        if (!Backtrack::TimeLimitRecive() || !entity->isOtherEnemy(localPlayer.get()))
        {
