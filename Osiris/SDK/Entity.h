...
enum EffectFlag
{
    BoneMerge = 1 << 0, // Performs bone merge on client side
    BrightLight = 1 << 1, // Dlight centered at entity origin
    Flashlight = 1 << 2, // Player flashlight
    NoInterp = 1 << 3, // Don't interpolate the next frame
    NoShadow = 1 << 4, // Don't cast shadow
    NoDraw = 1 << 5, // Don't draw entity
    NoRecieveShadow = 1 << 6, // Don't receive shadow
    BoneMergeFastCull = 1 << 7, // For use with EffectFlag_BoneMerge
    ItemBlink = 1 << 8, // Blink an item so that the player notices it
    ParentAnimates = 1 << 9, // Always assume that the parent entity is animating
    FastReflection = 1 << 10, // Marks an entity for reflection rendering when using $reflectonlymarkedentities material variable
    NoShadowDepth = 1 << 11, // Indicates this entity does not render into any shadow depthmap
    NoShadowDepthCache = 1 << 12, // Indicates this entity cannot be cached in shadow depthmap and should render every frame
    NoFlashlight = 1 << 13,
    NoCSM = 1 << 14 // Indicates this entity does not render into the cascade shadow depthmap
};

enum HideHudType {
    WEAPONSELECTION	= 1 << 0,	// Hide ammo count & weapon selection
    FLASHLIGHT = 1 << 1,
    ALL = 1 << 2,
    HEALTH = 1 << 3,	// Hide health & armor / suit battery
    PLAYERDEAD = 1 << 4,	// Hide when local player's dead
    NEEDSUIT = 1 << 5,	// Hide when the local player doesn't have the HEV suit
    MISCSTATUS = 1 << 6,	// Hide miscellaneous status elements (trains, pickup history, death notices, etc)
    CHAT = 1 << 7,	// Hide all communication elements (saytext, voice icon, etc)
    CROSSHAIR = 1 << 8,	// Hide crosshairs
    VEHICLE_CROSSHAIR = 1 << 9,	// Hide vehicle crosshair
    INVEHICLE = 1 << 10,
    BONUS_PROGRESS = 1 << 11,	// Hide bonus progress display (for bonus map challenges)
    RADAR = 1 << 12,   // Hides the radar in CS1.5
    MINISCOREBOARD = 1 << 13   // Hides the miniscoreboard in CS1.5
};
...
public:
    ...
    VIRTUAL_METHOD(void, release, 1, (), (this + sizeof(uintptr_t) * 2))
        ...
        VIRTUAL_METHOD(csgo::Team, getTeamNumber, 88, (), (this))
    ...
    bool& readyToDraw() noexcept
    {
        return *reinterpret_cast<bool*>(reinterpret_cast<uintptr_t>(this) + 0x274);
    }
    ...
    int& hideHUD() noexcept { return *reinterpret_cast<int*>(this + 0x3014); }
    ...
    NETVAR(mins, "CBaseEntity", "m_vecMins", Vector)
    NETVAR(maxs, "CBaseEntity", "m_vecMaxs", Vector)
    ...
    NETVAR(healthShotBoostExpirationTime, "CCSPlayer", "m_flHealthShotBoostExpirationTime", float)
    NETVAR(observerMode, "CBasePlayer", "m_iObserverMode", ObsMode)

    NETVAR(statsKills, "CCSPlayer", "m_iMatchStats_Kills", int)
    NETVAR(statsAssists, "CCSPlayer", "m_iMatchStats_Assists", int)
    NETVAR(statsDeaths, "CCSPlayer", "m_iMatchStats_Deaths", int)
    NETVAR(statsDamage, "CCSPlayer", "m_iMatchStats_Damage", int)
    NETVAR(statsUtilityDamage, "CCSPlayer", "m_iMatchStats_UtilityDamage", int)
    NETVAR(statsHeadShotKills, "CCSPlayer", "m_iMatchStats_HeadShotKills", int)
    ...
    {
        return std::max(memory->globalVars->currenttime - getFlashStartTime(), 0.0f);
    }
    ...
    [[nodiscard]] bool isDucking() noexcept
    {
        return (flags() & 2) != 0;
    }
};
