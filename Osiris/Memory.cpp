...
static std::uintptr_t findPattern(ModuleInfo moduleInfo, std::string_view pattern) noexcept
{
    ...
    if constexpr (ReportNotFound)
        MessageBoxA(nullptr, ("Failed to find pattern:\n" + patternToString(pattern)).c_str(), "Azurre", MB_OK | MB_ICONWARNING);
#endif
    return 0;
}
...
std::uintptr_t MemoryN::findPatternA(const char* moduleName, std::string_view pattern) noexcept {
    return findPattern(moduleName, pattern);
}

#define inRange(x,a,b)    (x >= a && x <= b)
#define getBits( x )    (inRange((x&(~0x20)),'A','F') ? ((x&(~0x20)) - 'A' + 0xa) : (inRange(x,'0','9') ? x - '0' : 0))
#define getByte( x )    (getBits(x[0]) << 4 | getBits(x[1]))
DWORD MemoryN::findPatternS(void* start, DWORD size, std::string pattern) {
...
Memory::Memory() noexcept {
    ...
    scopeBlurry = findPattern(CLIENT_DLL, "\xFF\xB7????\x8B\x01\xFF\x90????\x8B\x4C\x24\x24") - 6;
    scopeLines = findPattern(CLIENT_DLL, "\x0F\x82????\xFF\x50\x3C") + 9;
    ...
    scopeRemove = findPattern(CLIENT_DLL, "\x55\x8B\xEC\x83\xE4?\x83\xEC?\x56\x89\x4C\x24?\x8B\x4D");
    ...
    setVisualsDataTarget = findPattern(CLIENT_DLL, "\x55\x8B\xEC\x81\xEC????\x53\x8B\xD9\x56\x57\x8B\x53\x5C");
    ...
    smokeCount = *reinterpret_cast<uint32_t**>(findPattern(CLIENT_DLL, "\xA3????\x57\x8B\xCB") + 1);
    bombText = findPattern(CLIENT_DLL, "\x55\x8B\xEC\x8B\x45\x08\x85\xC0\x53");

    netGraph = findPattern(VGUIMATSURFACE_DLL, "\x55\x8B\xEC\x81\xEC????\x56\x57\xFF");
    netGraphFPS = (void*)(findPattern(CLIENT_DLL, "\x8B\x8F????\x83\xC4\x28"));
    netGraphLoss = (void*)(findPattern(CLIENT_DLL, "\xF3\x0F\x10\x44\x24?\x8D\x84\x24????\x0F\x5A\xC0\x83\xC4"));
    
    SFUI = findPattern(CLIENT_DLL, "\x55\x8B\xEC\x56\x8B\xF1\x83\xBE?????\x75\x4B");
    crosshairOutline = (findPattern(CLIENT_DLL, "\xFF\x50\x3C\xF3\x0F\x10\x4D?\x66\x0F\x6E\xC6") + 3);
    radarHLTV = findPattern(CLIENT_DLL, "\x55\x8B\xEC\x56\x8B\xF1\x80\x7E\x0C\x00\x0F\x84");
    particleCollection = relativeToAbsolute<decltype(particleCollection)>(findPattern(CLIENT_DLL, "\xE8????\x8B\x0E\x83\xC1\x10") + 1);
    getAccountData = relativeToAbsolute<decltype(getAccountData)>(findPattern(CLIENT_DLL, "\xE8????\x85\xC0\x74\xEE") + 1);

    uint32_t steamUserHandle = ((uint32_t(__cdecl*)(void))GetProcAddress(GetModuleHandleA("steam_api.dll"), "SteamAPI_GetHSteamUser"))();
    uint32_t steamPipeHandle = ((uint32_t(__cdecl*)(void))GetProcAddress(GetModuleHandleA("steam_api.dll"), "SteamAPI_GetHSteamPipe"))();
    steamClient = ((ISteamClient * (__cdecl*)(void))GetProcAddress(GetModuleHandleA("steam_api.dll"), "SteamClient"))();
    steamUser = (SteamUser*)steamClient->GetISteamUser(steamUserHandle, steamPipeHandle, "SteamUser019");
    steamUserStats = steamClient->GetISteamUserStats(steamUserHandle, steamPipeHandle, "STEAMUSERSTATS_INTERFACE_VERSION011");
    steamMatchmaking = steamClient->GetISteamMatchmaking(steamUserHandle, steamPipeHandle, "SteamMatchMaking009");
    steamIFriends = steamClient->GetISteamFriends(steamUserHandle, steamPipeHandle, "SteamFriends017");
    steamGameCoordinator = (SteamGameCoordinator*)steamClient->GetISteamGenericInterface(steamUserHandle, steamPipeHandle, "SteamGameCoordinator001");

    initKeyValues = (fInitKeyValues)findPattern(CLIENT_DLL, "\x55\x8B\xEC\x51\x33\xC0\xC7\x45");
    partyList = *(PartyList***)(findPattern(CLIENT_DLL, "\xEB\x35\x8D\x04\x92") + 0x2 + 0x35 + 0x1);
    matchmakingUIStatusString = *(const char***)(findPattern(CLIENT_DLL, "\x8B\xF1\xB9????\x6A\x00") + 3);
    currentMatchState = findPattern(CLIENT_DLL, "\xA1????\x8B\x4D\x08\xD1\xE8") + 1;
    partyBrowser = *(PartyBrowser***)(findPattern(CLIENT_DLL, "\x83\xE4\xF8\x8B\x0D????\x85\xC9") + 5);
    beenInvited = relativeToAbsolute<decltype(beenInvited)>(findPattern(CLIENT_DLL, "\x50\xE8????\x85\xC0\x74\x37") + 2);
    joinLobby = (memoryFunction)findPattern(CLIENT_DLL, "\x55\x8B\xEC\x83\xE4\xF8\x8B\x0D????\x85\xC9");
    joinLobbyID = (uint64_t*)*(DWORD*)(findPattern(CLIENT_DLL, "\xA1????\x0B\x05????\x74\x13") + 1);

    createSession = relativeToAbsolute<decltype(createSession)>(findPattern(CLIENT_DLL, "\xE8????\x83\xEC\x14\xE8") + 1);
    createSessionUI = relativeToAbsolute<decltype(createSessionUI)>(findPattern(CLIENT_DLL, "\xE8????\x8B\x50\x08") + 1);

    invitePlayer = findPattern(CLIENT_DLL, "\x83\x3D?????\x74\x5E\x8B\x0D????\x68") + 2;
    invitePlayerFunction = findPattern(CLIENT_DLL, "\x55\x8B\xEC\x83\xE4\xF8\x83\xEC\x1C\x53\x56\x57\x8B\xF1\xFF\x15????\x8B");

    keyValuesSystem = reinterpret_cast<KeyValuesSystem * (STDCALL*)()>(GetProcAddress(GetModuleHandleW(L"vstdlib"), "KeyValuesSystem"))();
    ...
    keyValuesGetString = (void*)(findPattern(CLIENT_DLL, "\x55\x8B\xEC\x83\xE4\xC0\x81\xEC????\x53\x8B\x5D\x08"));
    ...
    overheadInfo = reinterpret_cast<void*>(findPattern(CLIENT_DLL, "\xE8????\x3B\xC6\x5E\x5F") + 5);
